# Edited by me. Uses manimlib, changing for manim community version.
from manim import *
import numpy as np
...
class FerrersDiagram(VGroup):
    def __init__(self, partition_sequence, padding = .3, center_x = 0, center_y = 0, color = WHITE, shape = "circle"):
        ...
        # self.color = color # !FIXME: the color function is not working
        ...
        for i in range(np.size(partition_sequence)):
            ...
            for j in range(cell):
                ...
                dot.move_to(np.array([center_x - 0.5 * padding * np.max(partition_sequence) + padding * j, center_y - .5 * padding * np.size(partition_sequence) + padding * (np.size(partition_sequence)-i), 0]))
                self.add(dot)
...
class _ShiftALayerOnce(AnimationGroup):
    def __init__(self, ferrer, layer_to_shift):
        ...
        super().__init__(*animations, run_time=.2) # !IMPORTANT: time seems to subtract from animation
                                                  # from _ShiftALayerCompletely or something! not sure it's that!
...
class _ShiftALayerCompletely(Succession):
    def __init__(self, ferrer, layer_to_shift):
        ...
        super().__init__(*animations) # adding run_time here will delay between _ShiftLayerOnce
    def check_if_input_is_ferrers_diagram(self, ferrer):
        if not isinstance(ferrer, FerrersDiagram):
            raise Exception("Convolution must take in a Ferrer's Diagram object")
...
class _Justify(AnimationGroup):
    def __init__(self, ferrer):
        ...
        super().__init__(*animations, run_time=0.5) # adding a 1 second run_time for justifying
    def check_if_input_is_ferrers_diagram(self, ferrer):
        if not isinstance(ferrer, FerrersDiagram):
            raise Exception("Convolution must take in a Ferrer's Diagram object")
...
class Convoluting(Succession):
    ...
    def __init__(self, ferrer):
        ...
        ferrer.updatePaddingDistance() # seems to work without this
        ferrer.updateLayers()          # absolutely necessary

        # ?REVIEW: 
        # I DO NOT UNDERSTAND THE PURPOSE OF UPDATING A NEW PARTITION SEQUENCY TO USE ONE AGAIN
        # CODE SEEMS TO RUN WITHOUT IT
...
class FerrersDiagramDemonstration(Scene):
    # arguments = {
    #     "camera_config": {"background_color": "#833f14"} # seems to do nothing
    # }
    def construct(self):
        ...
        # ferrers_diagram4 = FerrersDiagram(partition_sequence=np.array([8,7,5,5,3,1]), shape="square", color="#e87335", center_x=3, center_y=-2)
        ...
        # animations
        #self.play(Create(ferrers_diagram), Create(ferrers_diagram3), Create(ferrers_diagram4))
        #self.play(FranklinInvoluting(ferrers_diagram), SortingParts(ferrers_diagram3), Convoluting(ferrers_diagram4))
